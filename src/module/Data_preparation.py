...
def quantity_executors(data):
    ...
    group.columns = ['ИНН исполнителя','Сумма контрактов', 'Средняя цена контракта',
                     'Сумма оплат', 'Средняя сумма оплат', 'Количество контрактов', 'Прибыль(убыток)', 'Количество прекращенных контрактов', 'Неустойки (штрафы и пени)', 'Включение в реестр недобросовестных поставщиков']
    return group
